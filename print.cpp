...
void VGAX::print(byte *fnt, byte glyphscount, byte fntheight, byte hspace, byte vspace, const char *str, char dx, char dy0, byte color) {
  char dx0=dx, dy;
  char *pstr=(char*)str;
  char c;
  while (c=pgm_read_byte(pstr++)) {
    if (c=='\n') {
      ...
      if (c>=0 && c<glyphscount) {
        ...
        //(inline of bitblit(fntg+1, fntw, fntheight, dx, dy, color & 3)) to save from stack overflow
